...
람다를 단순히 익명 클래스로 치환하여 해석할 경우 익명 클래스의 특징을 그대로 가져가게 된다.

Java 아키텍터들도 람다식 표현을 위해 많은 고민을 진행했다. Java에는 함수를 표현할 타입이 존재하지 않기 때문에 기존에 사용하던 추상 메서드가 하나인 인터페이스를 활용한 방법을 고민하였다. 이것을 기반으로 Java 8에서 함수형 인터페이스라는 개념을 통해 람다 표현식을 작성할 수 있게 되었다. 
