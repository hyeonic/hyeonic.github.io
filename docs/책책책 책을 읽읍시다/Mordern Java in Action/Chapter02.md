...
위 방식도 나름 좋은 해결책이다. 하지만 구현 코드는 대부분 중복된다. 이것은 소프트웨어 공학의 **DRY<sup>don't repeat yourself</sup> 원칙** 을 어기는 것이다. 또한 탐색 성능 개선을 위해서는 구현 코드를 전부 수정해야 한다. 이것은 비싼 대가를 야기한다.
